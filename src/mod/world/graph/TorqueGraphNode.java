...
public class TorqueGraphNode extends GraphNode<TorqueGraph>{
    ...
    public void displayBars(Table table){
        ...
        table.row();
        table.add(new Bar(() -> Core.bundle.format("bar.mod-torquespeed", Strings.fixed(n1.graph.lastVelocity*10f, 1)), () -> Pal.ammo, () -> Mathf.clamp(n1.graph.lastVelocity/Math.max(0.01f,maxSpeed))));
    }
    ...
    public void setStats(Stats stats){
        addLevelStat(stats, modStat.friction, baseFriction * 60f, new float[]{0.5f, 1f, 2f, 5f, 10});
        stats.add(modStat.inertia, baseInertia);
        if(maxSpeed > 0) stats.add(modStat.maxSpeed, maxSpeed * 10f, UnityStatUnit.rpm);
        if(maxTorque > 0) stats.add(modStat.maxTorque, maxTorque, UnityStatUnit.torque);
    }
}
